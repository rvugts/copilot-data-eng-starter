...
- **Add more context** — user provides additional info (e.g. attach existing docs, ADRs, or architecture diagrams via `@` references in Copilot Chat)
...
This skill is the entry point for the SDD workflow in this repo:

| Step | Skill / command |
|------|----------------|
| Draft the spec | `/create-spec` — this skill; writes `docs/specs/spec.md` |
| Decompose into tasks | `/create-tasks` — reads `@docs/specs/spec.md`, writes `docs/specs/tasks.md` |
| Author prompts per task | `/create-prompt` — uses task prompt hints; saves numbered files under `./prompts/` |
| Execute tasks | `/run-prompt` — runs saved prompts sequentially |

Tips for best results in Copilot Chat:

- **Attach context with `@`.** During Phase 3, attach existing architecture docs, ADRs, or relevant source files via `@` references so questions are grounded in the real codebase.
- **Reference the template.** Attach `@docs/spec.template.md` in your message for section-by-section alignment during generation.
- **Downstream chain.** After generation, follow up with `/create-tasks` to produce the ordered task breakdown at `docs/specs/tasks.md`.
