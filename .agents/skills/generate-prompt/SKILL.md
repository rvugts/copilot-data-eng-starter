...
description: Create a new prompt file that another agent can execute. Use when the user wants to create a prompt, generate a task prompt, or needs a reusable executable instruction saved to the prompts directory.
argument-hint: Describe the prompt you want to create
---

<context>
Paths are relative to the workspace root. Before generating prompts, use `file_search` with pattern `prompts/*.md` (or `list_dir` on the workspace-root absolute path for `./prompts/`) to:
...
IF no task description was provided (user invoked the generate-prompt skill without details):
→ **IMMEDIATELY ask the user** using `vscode_askQuestions`:
...
Generate 2-4 questions based ONLY on genuine gaps. Use `vscode_askQuestions` with `options` arrays for structured choices.
...
<decision_gate>
After receiving answers, use `vscode_askQuestions` to present the decision gate:
...
4. **Required tools**: File references, terminal commands, MCP servers
...
Create the prompt(s) and save to the prompts folder using the `create_file` tool. When calling tools that require filesystem paths, resolve each `./...` path to its workspace-root absolute path before invoking the tool.
...
- File references inside generated prompts should use markdown links for files when practical, for example `[package.json](./package.json)`, and explicit relative folder paths when a folder reference is needed
- Reference to reading `.github/copilot-instructions.md` for project conventions — only if such a file exists in the project; omit if not present
...
- Terminal command execution when system state matters
- MCP server references when specifically requested or obviously beneficial

Output Format:

1. Generate prompt content with XML structure
2. Save to: `./prompts/[number]-[descriptive-name].md` using `create_file` with the workspace-root absolute target path
   - Number format: 001, 002, 003, etc. (use `file_search` on `prompts/*.md`, excluding `./prompts/completed/`, to determine next number)
...
Reference relevant files with markdown links such as [example](./path/to/file.ext)
Reference relevant folders with explicit relative paths such as `./src/components/`
</context>
...
<data_sources>
Reference files with markdown links such as [example](./path/to/file.ext)
Reference folders with explicit relative paths such as `./src/data/`
...
5. **Context Loading**: Only request file reading when the task explicitly requires understanding existing code. Use patterns like:
   ...
   - "Use read_file to examine `package.json` for dependencies" (when adding new packages)
   - "Use list_dir to review `src/database/` for schema files" (when modifying data layer)
...
7. **Tool Integration**:
   ...
   - Use terminal commands for environment checking when state matters
...
<decision_tree>
After saving the prompt(s), use `vscode_askQuestions` to present the following decision tree:
...
- Files saved to ./prompts/[number]-[name].md with correct sequential numbering using `create_file`
- Decision tree presented to user based on single/sequential scenario
- User choice executed (files opened or the run-prompt workflow started as requested)
</success_criteria>

<meta_instructions>

- **Intake first**: Complete step_0_intake_gate before generating. Ask for structured clarification using `vscode_askQuestions`.
- **Decision gate loop**: Keep asking questions until user selects "Proceed"
- Use `file_search` with pattern `prompts/*.md` (excluding `./prompts/completed/`) to find existing prompts and determine next number in sequence
- If ./prompts/ doesn't exist, create the first prompt (parent directories will be created automatically by `create_file`)
