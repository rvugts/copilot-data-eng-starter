...
- One staging model: `stg_raw__orders` from `{{ source('raw', 'orders') }}`
...
|----|-------------|---------------------|----------|
| FR-1 | Staging model exists | `models/staging/stg_raw__orders.sql` compiles with `dbt parse` | Must Have |
...
- [ ] `dbt build --select stg_raw__orders` passes in dev
